...
function scoreBadge(score: number): string {
    if (score >= 90) return `<span style="background:#0d652d;color:#fff;padding:3px 10px;border-radius:12px;font-size:13px;font-weight:600;">🟢 ${score}% — Apply Now</span>`;
    if (score >= 75) return `<span style="background:#1a73e8;color:#fff;padding:3px 10px;border-radius:12px;font-size:13px;font-weight:600;">🔵 ${score}% — Strong Match</span>`;
    if (score >= 50) return `<span style="background:#e8a317;color:#fff;padding:3px 10px;border-radius:12px;font-size:13px;font-weight:600;">🟡 ${score}% — Consider</span>`;
...
export function buildDigestHtml(jobs: DigestJob[], stats: DigestStats, isLowMatchWeek: boolean): string {
